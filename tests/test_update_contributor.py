...
import update_contributor as update_contributor  # noqa
...
class TestUpdateContributor(unittest.TestCase):
    ...
    def test_lambda_handler_happy_path(self, mock_create_engine, mock_update, mock_marsh, mock_alchemy_funks):  # noqa
    ...
    def test_update_contributor_fail(self, mock_create_engine, mock_alchemy_funcs):
        with mock.patch.dict(
            ...
            assert(x["statusCode"] == 500)
            assert ("Operation Error, Failed To Update Data: contributor_survey_period" in x['body']['Error'])
